...
import FeedbackMedal from "./FeedbackMedal";
import { useAuth } from "@clerk/nextjs";


const AppointmentCard = ({ booking, mode, isPast = false }) => {
  const [feedbackeOpen, setFeedbackOpen] = useState(false);
  const { has } = useAuth();
  ...
  return <>
    ...
    <article className="group relative bg-[#0f0f11] border border-white/10 transition-all duration-300
      hover:-translate-y-0.5 rounded-2xl bg-linear-to-t from-transparent via-transparent to-amber-300/10
      p-7 flex flex-col gap-6 self-start">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-sm text-stone-300" suppressHydrationWarning>{formatDate(startTime)}</p>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-sm text-stone-300" suppressHydrationWarning>
